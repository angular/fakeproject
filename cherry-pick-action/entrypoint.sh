...
if [ $(/containsLabel $GITHUB_EVENT_PATH "target: patch") == "true" ]; then
  PATCH_BRANCH=$(git ls-remote --heads | grep -E 'refs\/heads\/[0-9]+\.[0-9]+\.x' | cut -d '/' -f3 | sort -r | head -n1)
fi
if [ $(/containsLabel $GITHUB_EVENT_PATH "target: minor") == "true" ]; then
  MINOR_BRANCH=$(git ls-remote --heads | grep -E 'refs\/heads\/[0-9]+\.x' | cut -d '/' -f3 | sort -r | head -n1)
fi
...
if [ -z PATCH_BRANCH ]; then
  if [ -z MINOR_BRANCH ]; then
    if [ -z MAJOR_BRANCH ]; then
      echo "no branch targeted"
      exit 0;
    fi
  fi
fi
